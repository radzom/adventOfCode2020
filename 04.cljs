...
(def passports (->> data
                    ...
                    (map #(str/split %1 #" "))
                    (map (partial map #(str/split %1 #":")))
                    (map #(into (sorted-map) %1))
                    (map w/keywordize-keys)))

(def needed-keys [:byr :eyr :iyr :hgt :hcl :ecl :pid])

(defn passport-1? [pp] (apply = (map count [needed-keys (select-keys pp needed-keys)])))

(defn byr? [byr]
  (let [year (re-matches #"^\d{4}$" byr)]
    (and year (<= 1920 (js/parseInt year 10) 2002))))

(defn iyr? [iyr]
  (let [year (re-matches #"^\d{4}$" iyr)]
    (and year (<= 2010 (js/parseInt year 10) 2020))))

(defn eyr? [eyr]
  (let [year (re-matches #"^\d{4}$" eyr)]
    (and year (<= 2020 (js/parseInt year 10) 2030))))

(defn hgt? [hgt]
  (let [cm (re-matches #"^\d{3}(?:cm)$" hgt)
        in (re-matches #"^\d{2}(?:in)$" hgt)]
    (or (and (not (nil? cm)) (<= 150 (js/parseInt cm 10) 193))
        (and (not (nil? in)) (<= 59 (js/parseInt in 10) 76)))))

(defn hcl? [hcl] (re-matches #"^#[0-9a-f]{6}$" hcl))

(defn ecl? [ecl] (re-matches #"^(?:amb)|(?:blu)|(?:brn)|(?:gry)|(?:grn)|(?:hzl)|(?:oth)$" ecl))

(defn pid? [pid] (re-matches #"^\d{9}$" pid))
...
(println (count (filter passport-1? passports))) ;; 210
(println (count (filter passport-2? passports))) ;; 131
