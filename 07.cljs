...
(def rules ["wavy turquoise bags contain no other bags."
...
(defn bag->keyword [bag-str] (keyword (str/replace bag-str #" " "-")))

(defn bag-count->entry [bag-count-str] (if (= bag-count-str "no other bags")
                                         nil
                                         (let [words (str/split bag-count-str #" ")]
                                           {(bag->keyword (str/join " " (take 2 (drop 1 words))))
                                            (js/parseInt (first words) 10)})))

(defn scanner [line] (let [regex (js/RegExp "^(\\w+ \\w+) bags contain ((?:(?:no other bags))|(?:(?:\\d \\w+ \\w+ bags?)(?:, )?)+)\\.$" "g")
                           matches (. regex exec line)
                           key (bag->keyword (second matches))
                           value (reduce conj {} (map bag-count->entry (str/split (nth matches 2) #", ")))]
                       {key value}))

(def bag-infos (reduce conj {} (map scanner rules)))

(defn contains-thing? [thing bag] (let [inner-bags (bag-infos bag)]
                                   (if (contains? inner-bags thing)
                                     true
                                     (some identity (map (partial contains-thing? thing) (keys inner-bags))))))

(defn count-bags [bag] (let [inner-bags (bag-infos bag)]
                            (reduce + 0 (map (fn [[key count]] (* count (+ 1 (count-bags key)))) inner-bags))))


(println (count (filter identity (map (partial contains-thing? :shiny-gold) (keys bag-infos))))) ;; 222

(println (count-bags :shiny-gold)) ;; 13264
